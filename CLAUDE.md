...
- Vite 7+ 构建
- Tailwind CSS 4.x（`@theme` 定义颜色 token）
- JavaScript（非 TypeScript）
- Linear App 风格，支持亮暗主题切换，默认亮色（优先跟随系统偏好）

### 核心视觉规则
1. **色彩克制**：90% 灰度体系，彩色只用于主操作、关键状态、数据高亮，同一视图彩色不超过 2-3 处
2. 颜色只用 `linear-*` token，禁止 Tailwind 默认色和硬编码 `text-white`/`bg-black`
3. 主题优先级：跟随系统 `prefers-color-scheme` → 用户手动切换 → 默认亮色。`[data-theme="dark"]` 覆盖为暗色
4. 玻璃态卡片：`bg-linear-bg-secondary/40 backdrop-blur-md rounded-3xl border border-linear-border/50`
5. 强调色渐变：紫(`#A78BFA`) → 粉(`#F472B6`)，辉光用 `blur` + 低透明度
6. 动画克制：`transition-all duration-300`，入场用 `fade-in-up`，数据用 SVG 绘制动画
7. 主题切换用 `useTheme` composable（`data-theme` 属性 + localStorage 持久化 + 系统偏好检测）
